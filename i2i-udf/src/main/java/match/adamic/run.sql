...
create table if not exists adamic_i2i (
    item_id string,
    sim_items string
)
;

insert overwrite table adamic_i2i
...
--计算topk推荐结果
create table if not exists adamic_topk_rec (
    user_id string,
    item_id string,
    score double,
    seq bigint
...
insert overwrite table adamic_topk_rec
...
--测评召回率
--hr@20 = 0.0721456603084786
--hr@100 = 0.24144952998083416

select count(b.user_id) / count(1) as hr
from ml_1m_data_test a
left outer join (
    select *
    from adamic_topk_rec
    where seq <= 100
) b
on a.user_id = b.user_id
