...
CREATE TEMPORARY FUNCTION llr_cf_udaf as "match.llr.LLRUDAF" using jar "/opt/hive-0.0.2-SNAPSHOT.jar";


--i2i计算
create table if not exists llr_cf_i2i (
    item_id string,
    sim_items string
)
;

insert overwrite table llr_cf_i2i

select split(item,':')[0] as item_id, llr_cf_udaf(item_neighbors, cast(split(item,':')[1] as bigint))
from
(
...
--计算topk推荐结果
create table if not exists llr_cf_topk_rec (
    user_id string,
    item_id string,
    score double,
    seq bigint
...
insert overwrite table llr_cf_topk_rec
...
--测评召回率
--hr@20 = 0.0765777585105412
--hr@100 = 0.26589737154330567

select count(b.user_id) / count(1) as hr
from ml_1m_data_test a
left outer join (
    select *
    from llr_cf_topk_rec
    where seq <= 100
) b
on a.user_id = b.user_id
